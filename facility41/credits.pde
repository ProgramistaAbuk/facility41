...
void credits_setup() {
    ...
    credits_button.put("back", new Object[]{ (50), 30, 125, 50, "Back", 18.0f, color(0,0,0), color(255,255,255), color(0,0,0), -1 });
}
...
void credits() {
  ...
  image(credits, bg_x, 0);
    ...
    if (mouseX > x && mouseX < x+w && mouseY > y && mouseY < y+h) {
        credits_button.put(id, menu_makeButton(x,y,w,h,txt,txtSz,btnC,txtC, border, alpha_count, true)) ;
    } else {
        credits_button.put(id, menu_makeButton(x,y,w,h,txt,txtSz,btnC,txtC, border, alpha_count, false)) ;
    }
